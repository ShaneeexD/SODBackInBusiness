    ...
    internal class EmployeeDetailsView
    {
        ...
        public void Build(GameObject parent)
        {
            ...
            if (clonedCard != null)
            {
                ...
                // ---------------------
                // Work Rota anchored to BIB_ContentsPage (the actual GameObject with Image component)
                workRotaText = CreateTMP(bibContentsPage, "Emp_WorkRota", "Work Schedule", TextAlignmentOptions.TopLeft);
                var rotaRt = workRotaText.GetComponent<RectTransform>();
                ...
                // Log the hierarchy path for debugging
                Plugin.Logger.LogInfo($"EmployeeDetailsView: Work Rota created with path: {GetGameObjectPath(workRotaText.gameObject)}");
                Plugin.Logger.LogInfo($"EmployeeDetailsView: Work Rota parent is: {workRotaText.transform.parent?.name ?? "null"}");
        ...
        private void OnFireClicked()
        {
            try
            {
                ...
                // Show UniverseLib Yes/No panel (no background; ClearSearch-styled buttons)
                global::BackInBusiness.BusinessUIManager.Instance.ShowFireYesNo(
                    "Confirm Firing",
                    ...
                    onRight: () =>
                    {
                        try
                        {
                            ...
                            if (emp != null)
                            {
                                // Use our AIGoalManager to handle making the employee unemployed and homeless
                                try
                                {
                            ...
                            {
                                Plugin.Logger.LogWarning("OnFireClicked.onRight: CurrentEmployee was null at confirm time.");
                            }
        ...
        public void Show(Citizen citizen, Occupation occ, int addressId = -1, int rosterIndex = -1)
        {
            try
            {
                ...
                try
                {
                    ...
                    Plugin.Logger.LogInfo($"Work Rota rect size={workRotaText.rectTransform.rect.size}");
                }
                catch { }
